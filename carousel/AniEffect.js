/* 
	...
	animate(ochangs,{left: -1000*nIndex}, 1000,4);
...
var AniEffect = {
	...
	zfBounce: {//指数衰减的反弹缓动。
		easeIn: function(t,b,c,d){
			return c - AniEffect.zfBounce.easeOut(d-t, 0, c, d) + b;
		},
		easeOut: function(t,b,c,d){
			if ((t/=d) < (1/2.75)) {
		...
		easeInOut: function(t,b,c,d){
			if (t < d/2) return AniEffect.zfBounce.easeIn(t*2, 0, c, d) * .5 + b;
			else return AniEffect.zfBounce.easeOut(t*2-d, 0, c, d) * .5 + c*.5 + b;
		}
	}
}
////////////////////以上都是算法
...
function animate(ele,obj,nDuration,effectType,fnCallback){
	...
	//effectType 1,2,3,4
	if(!effectType){
		var fn=AniEffect.zfLinear;//直线，匀速的 1
		...
		switch(effectType){
			case 1:
				...
				break;
			case 2:
				var fn=AniEffect.Quart.easeInOut;//直线，匀速的 1
				break;
			case 3:
				var fn=AniEffect.Elastic.easeOut;//直线，匀速的 1
				break;
			case 4:
				var fn=AniEffect.zfBounce.easeOut;//直线，匀速的 1
				break;
			default:
		...
		var fn=AniEffect.zfLinear;	
		}else{			
			var fn=AniEffect[effectType[0]][effectType[1]];		
		}		
